...
class SignalGenerator:
    ...
    def generate_signals(self, data):
        ...
        price_above_ma = (df['Close'] > df[f'MA{ma_long}']) & (df['Close'].shift(1) <= df[f'MA{ma_long}'].shift(1))
        
        # 价格跌破长期均线: 价格从长期均线上方跌破至下方
        price_below_ma = (df['Close'] < df[f'MA{ma_long}']) & (df['Close'].shift(1) >= df[f'MA{ma_long}'].shift(1))
        
        signals.loc[price_above_ma, 'Price_MA_Relation'] = 1
        signals.loc[price_below_ma, 'Price_MA_Relation'] = -1
